...
bool FullGameCompiler::link_results(PackageWriter& writer) {
    ...
    // Stage 11: Emit ItemData chunk (ITDT).
    // Source: Crystal ItemAttributes table at profile_.offsets.item_attributes.
    // Each raw entry is semanticized to HeldItemEffectType in the extractor.
    // No raw Crystal HELD_* values survive into the runtime.
    // Fail-closed: ROM bounds failure -> FATAL.
    {
        auto item_result = crystal::extract_all_items(rom_, profile_);
        if (!item_result.success) {
            std::cerr << "FATAL: Stage 11 -- ItemData extraction failed: "
                      << item_result.error << "\n";
            return false;
        }
        writer.add_item_data(item_result.items);
        std::cout << "  ItemData chunk: " << item_result.items.size() << " items\n";
    }
