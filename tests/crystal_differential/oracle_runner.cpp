...
// SameBoy identity constants -- inlined from sameboy_version.h.
// (Do not #include sameboy_version.h here: clang-cl 18 has a column-counting
//  bug with certain included headers that causes misparse of later identifiers.)
// These must match the values generated by CMake into sameboy_version.h.
#ifndef GB_VERSION
...
int runner_main(int argc, char* argv[], RunnerConfig defaults)
{
    ...
    // --- SameBoy identity checks (compile-time via sameboy_version.h) ----------
    // GB_COMMIT, GB_VERSION, GB_DIRTY are embedded by CMake at configure time.
    // These checks must happen before ANY output; errors use ERROR-N format.
    // startup_err is used for the 1-based deterministic error counter for all
    // startup (pre-case) failures.
    int startup_err = 0;
    auto startup_fail = [&](const char* subsys, const std::string& reason) -> int {
        ++startup_err;
        std::string line = fmt_error(startup_err,"STARTUP",subsys,"CONFIG_ERROR",reason);
        std::cerr << line << "\n";
        return EXIT_HARNESS_ERROR;
    };

    if (std::string(GB_COMMIT) != PINNED_SAMEBOY_COMMIT)
        return startup_fail("SAMEBOY",
            "commit mismatch: pinned="+std::string(PINNED_SAMEBOY_COMMIT)
            +" built="+GB_COMMIT);

    if (std::string(GB_VERSION) != PINNED_SAMEBOY_VERSION)
        return startup_fail("SAMEBOY",
            "version mismatch: pinned="+std::string(PINNED_SAMEBOY_VERSION)
            +" built="+GB_VERSION);

    // GB_DIRTY is non-empty if references/SameBoy had uncommitted changes
    // at CMake configure time. Fail closed -- dirty source tree is not the
    // pinned oracle. Re-run cmake after cleaning to refresh.
    if (std::string(GB_DIRTY).size() > 0)
        return startup_fail("SAMEBOY",
            "working tree has tracked modifications (re-run cmake to refresh): "
            +std::string(GB_DIRTY));
    ...
    std::vector<uint8_t> rom_bytes;
    {
        ...
        if (!f)
            return startup_fail("ROM", "cannot open: "+rom_path);
        rom_bytes.assign(std::istreambuf_iterator<char>(f), {});
    }
    if (rom_bytes.size() != CRYSTAL_ROM_SIZE)
        return startup_fail("ROM",
            "wrong size "+std::to_string(rom_bytes.size())
            +" expected "+std::to_string(CRYSTAL_ROM_SIZE));
    {
        ...
        if (sha != PINNED_ROM_SHA1) {
            std::cout << " MISMATCH\n";
            return startup_fail("ROM",
                "SHA-1 mismatch: expected="+std::string(PINNED_ROM_SHA1)+" actual="+sha);
        }
        std::cout << " OK\n";
    }
    ...
    SymCache sym;
    {
        ...
        std::cout << (std::string(PINNED_SYM_SHA1_CONST).empty()
                      ? " (not pinned -- anchor check only)" : " OK") << "\n";
    }
    ...
    // 3. Fixture address range validation.
    {
        ...
        if (!err.empty())
            return startup_fail("FIXTURES", err);
    }
    ...
    // Wall-clock timeout: each future is given WALL_CLOCK_TIMEOUT_S seconds.
    // If it expires, a synthetic HARNESS_ERROR result is recorded immediately
    // and f.get() is called afterward (may block until the worker finishes,
    // which is bounded by the insn cap -- the timeout is an emergency fallback).
    // =========================================================================
    ...
    for (size_t i=0; i<n; ) {
        ...
        for (size_t j=i; j<batch_end; ++j) {
            ...
            if (status != std::future_status::ready) {
                timed_out[j] = true;
                // f.get() will be called below; the insn cap bounds how long
                // it can still take. We record the timeout now so the result
                // slot gets a HARNESS_ERROR regardless of what f.get() returns.
            }
        }
        i = batch_end;
    }
    ...
    // Timed-out futures: f.get() is called (may still block briefly until the
    // worker's insn cap fires), then the result is overridden with HARNESS_ERROR.
    // =========================================================================
    std::mutex out_mutex;
    ...
    for (size_t i=0; i<n; i++) {
        CaseResult r = futures[i].get();  // always call get() -- never leak a future
        if (timed_out[i]) {
            // Override whatever partial result the worker produced.
            r.status     = Status::HARNESS_ERROR;
            r.stop_reason = "WALL_CLOCK_TIMEOUT";
            r.detail      = "case did not complete within "
                           + std::to_string(WALL_CLOCK_TIMEOUT_S) + "s wall-clock limit";
        }
    ...
    for (size_t i=0; i<n; i++) {
        ...
        if (r.status == Status::MATCH) {
            ...
            if (!r.detail.empty() && r.status==Status::ENGINEMON_MISMATCH) {
                ...
                std::string dline;
                int field_n = cerr_idx;  // first error number for this case
                ...
                std::string errline = fmt_error(cerr_idx, subsystem, r.move_name, cls, reason);
                line << "  " << errline << "\n";
                error_lines.push_back(errline);
            }
